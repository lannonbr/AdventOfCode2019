...
fs.promises.readFile(path.join(__dirname, "input.txt")).then(buf => {
  ...
  for (let i_ = 0; i_ < xSize; i_++) {
    grid[i_] = [];
    grid[i_].length = ySize;
  ...
  for (let f = 0; f < xSize; f++) {
    for (let g = 0; g < ySize; g++) {
      if (grid[f][g] === 0) grid[f][g] = " ";
      if (grid[f][g] === 1) grid[f][g] = "█";
    }
  }

  console.log(grid.map(f => f.join("")));
});
